...
const sendToEyes = async ({
  ...
  try {
    ...
    console.log(`eyes comparison started for test "${specName}" version: "${version}" at ${new Date()}`);
    await eyes.open(
      appName,
  ...
  await eyes.close();
  console.log(`eyes comparison succeed for test "${specName} ${version}" at ${new Date()}`);
};
